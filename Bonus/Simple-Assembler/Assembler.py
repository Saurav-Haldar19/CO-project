...
op={"add":"00000","sub":"00001","movi":"00010","mov":"00011","ld":"00100","st":"00101","mul":"00110","div":"00111","rs":"01000","ls":"01001","xor":"01010","or":"01011","and":"01100","not":"01101","cmp":"01110","jmp":"01111","jlt":"11100","jgt":"11101","je":"11111","hlt":"11010","addf":"10000","subf":"10001","movf":"10010","nor":"10011","nand":"10100","xnor":"10101","max":"10110","min":"10111"}
    ...
    if ":" not in line[0]:
        if "add" == line[0]:
            ...
            if len(num)>8:
                ...
                break
            elif float(line[2][1:])<0.25 and float(line[2][1:]!=0):
                error=True
            ...
            if len(line)!=4:
                ...
                t+=op["nor"]+"00"+reg[line[1]]+reg[line[2]]+reg[line[3]]
        elif "nand" == line[0]:
            if len(line)!=4:
                error=True
                error_name=f"Error in line {line_no} : and must contain 3 parameters"
                ...
                t+=op["nand"]+"00"+reg[line[1]]+reg[line[2]]+reg[line[3]]
        elif "xnor" == line[0]:
            if len(line)!=4:
                error=True
                error_name=f"Error in line {line_no} : xor must contain 3 parameters"
                ...
                t+=op["xnor"]+"00"+reg[line[1]]+reg[line[2]]+reg[line[3]]
        elif "max" == line[0]:
            if len(line)!=3:
                error=True
                error_name=f"Error in line {line_no} : div must contain 2 parameters"
                ...
                t+=op["max"]+"00000"+reg[line[1]]+reg[line[2]]
        elif "min" == line[0]:
            if len(line)!=3:
                error=True
                error_name=f"Error in line {line_no} : div must contain 2 parameters"
                ...
                t+=op["min"]+"00000"+reg[line[1]]+reg[line[2]]
        elif "hlt" == line[0]:
            if len(line)!=1:
                error=True
        ...
        if "add" == line[1]:
            ...
            if len(num)>8:
                ...
                error_name=f"Error in line {line_no} : Illegal immediate value (more than 8 bits)"
                break
            elif float(line[3][1:])<0.25 and float(line[3][1:]!=0):
                error=True
                error_name=f"Error in line {line_no} : Illegal immediate value"
                break
            elif line[2]=="FLAGS":
            ...
            if len(line)!=5:
                ...
                t+=op["nor"]+"00"+reg[line[2]]+reg[line[3]]+reg[line[4]]
        elif "nand" == line[1]:
            if len(line)!=5:
                error=True
                error_name=f"Error in line {line_no} : and must contain 3 parameters"
                ...
                t+=op["nand"]+"00"+reg[line[2]]+reg[line[3]]+reg[line[4]]
        elif "xnor" == line[1]:
            if len(line)!=5:
                error=True
                error_name=f"Error in line {line_no} : xor must contain 3 parameters"
                ...
                t+=op["xnor"]+"00"+reg[line[2]]+reg[line[3]]+reg[line[4]]
        elif "max" == line[1]:
            if len(line)!=4:
                error=True
                error_name=f"Error in line {line_no} : div must contain 2 parameters"
                ...
                t+=op["max"]+"00000"+reg[line[2]]+reg[line[3]]
        elif "min" == line[1]:
            if len(line)!=4:
                error=True
                error_name=f"Error in line {line_no} : div must contain 2 parameters"
                ...
                t+=op["min"]+"00000"+reg[line[2]]+reg[line[3]]
        elif "hlt" == line[1]:
            if len(line)!=2:
                error=True
                error_name=f"Error in line {line_no} : hlt must contain 1 parameters"
